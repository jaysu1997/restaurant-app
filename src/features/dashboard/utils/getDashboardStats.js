...
// 更新今天的餐點銷售份數和銷售額
function updateDishStats(itemSalesStats, order) {
  order.items.forEach((item) => {
    const { name, servings, unitPrice } = item;
...
// 先轉換今天餐點銷售數據，再排出熱銷餐點
function getTopDishesFromStats(itemSalesStats) {
  // 熱銷餐點排序方式(先比銷售數量，數量相同筆金額，金額相同比文字順序，取前5名)
  return Array.from(itemSalesStats, ([dishName, dishStats]) => ({
    name: dishName,
...
export function getDashboardStats(orders = []) {
  ...
  return {
    ...
    todayTopDishes: getTopDishesFromStats(itemSalesStats),
    hourlyOrderCounts,
    last7DaysStats,
  };
}
